...
class TrainedModel(object):
  ...
  def __init__(self, config, batch_size, checkpoint_dir_or_path=None,
    ...
    with tf.Graph().as_default():
      ...
      inputs_info = \
          tf.saved_model.utils.build_tensor_info(self._inputs)
      outputs_info = \
          tf.saved_model.utils.build_tensor_info(self._outputs)
      prediction_signature = (
          tf.saved_model.signature_def_utils.build_signature_def(
              inputs={'inputs': inputs_info},
              outputs={'outputs': outputs_info},
              method_name= tf.saved_model.signature_constants.PREDICT_METHOD_NAME))

      # builder = tf.compat.v1.saved_model.builder.SavedModelBuilder('export')
      ...
      # converter = tf.compat.v1.lite.TFLiteConverter.from_saved_model('export',
      #                                                                input_arrays=['Placeholder_1'],
      #                                                                output_arrays=['outputs'],
      #                                                                tag_set=[tf.saved_model.TRAINING, tf.saved_model.SERVING])
