...
The options:
  ...
  'exclusive_event_callback' is a function
	...
	if include and not exclude then exclude = All_events end  -- 4.6
...
local function consistentise_ticks(scores) -- 3.6
	...
	for ks,score in ipairs(scores) do
		new_scores[ks] = opus2score(to_millisecs(score2opus(score)))
	end
	return new_scores
end
...
Number2patch = readOnly{   -- General MIDI patch numbers:
[0]='Acoustic Grand',
[1]='Bright Acoustic',
[2]='Electric Grand',
[3]='Honky-Tonk',
...
Notenum2percussion = readOnly{   -- General MIDI Percussion (on Channel 9):
[33]='Metronome Click',
[34]='Metronome Bell',
...
function concatenate_scores(scores)
	...
	for i = 2,#input_scores do
		...
		local output_stats = score2stats(output_score)
...
function grep(score, t)
	...
	local itrack = 2 while itrack <= #score do
		...
		for k,event in ipairs(score[itrack]) do
			local channel_index = Event2channelindex[event[1]]
...
function merge_scores(scores)
	...
	for ks,input_score in ipairs(consistentise_ticks(scores)) do -- 3.6
		local new_stats = score2stats(input_score)
			...
			if channels_so_far[channel] then
				...
				for itrack = 2,#input_score do
					for k3,input_event in ipairs(input_score[itrack]) do
						local ci = Event2channelindex[input_event[1]]
...
function mix_opus_tracks(input_tracks) -- 5.5
	...
	local output_opus = score2opus(output_score)
	return output_opus[2]
end

function mix_scores(input_scores)
	local output_score = {1000, {}}
...
function midi2ms_score(midi)
	return opus2score(to_millisecs(midi2opus(midi)))
end

function midi2opus(s)
	if not s then s = '' end
...
function midi2score(midi)
	return opus2score(midi2opus(midi))
end

function play_score(score)
	if not score then return end
	local midi
	if score_type(score) == 'opus' then
		midi = opus2midi(score)
	else
...
function score2midi(score)
	return opus2midi(score2opus(score))
end

function score2stats(opus_or_score)
--[[ returns a table:
...
function segment(...)
	...
	local my_type = score_type(score)
	if my_type == '' then
		return new_score
	end
...
function to_millisecs(old_opus)   -- 6.7
